...
def get_ltk(path=None):
	...
	start_enc_cmds = filter(lambda opcode_length_data: opcode_length_data[0] == 0x2019, cmds)
	ltks = map(lambda opcode_length_data: binascii.hexlify(opcode_length_data[2])[-32:], start_enc_cmds)
...
def get_rand_addr(path=None):
	...
	records = get_records(path=path)
	cmds = get_cmds(records)
	set_rand_addr = filter(lambda opcode_length_data: opcode_length_data[0] == 0x2005, cmds)
	addrs = map(lambda opcode_length_data: binascii.hexlify(opcode_length_data[2])[-12:], set_rand_addr)
...
def get_cmds(records):
	...
	hci_cmds = filter(lambda hci_type_hci_data: hci_type_hci_data[0] == hci_uart.HCI_CMD, hci_uarts)
	return map(lambda hci_type_hci_data: hci_cmd.parse(hci_type_hci_data[1]), hci_cmds)
